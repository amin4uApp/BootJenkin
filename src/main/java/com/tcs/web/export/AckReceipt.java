...
public class AckReceipt {
	...
	public void export(HttpServletResponse response) throws DocumentException, IOException {
		...
		// Logo Path
		// Image img =
		// Image.getInstance("C:\\Users\\aminul\\Documents\\workspace-spring-tool-suite-4-4.13.0.RELEASE\\SpringBootTest\\src\\main\\resources\\static\\assets\\images\\logo1.jpg");
	...
	public void export1(HttpServletResponse response)/// throws DocumentException, IOException
	{
		...
		try {
			...
			document.add(layout);
			String imagePath="/usr/uploaded_files/";
			// File currentDirectory = new File("E:\\temp");
			 File currentDirectory = new File(imagePath);
			
			 File[] files = currentDirectory.listFiles();
			 String img2;
                    int id=std.getId();
                    String ackid=String.valueOf(id);
			   // Iterate through the files and find the image you want
			   for (File file : files) {
			    // if (file.getName().endsWith(".jpg"))
			  if (file.getName().startsWith(ackid))
			     {
			     ...
			     String  finalPath = currentDirectory +"/"+ img2;
			 Image im=Image.getInstance(finalPath);
			 //Image img = Image.getInstance("E:\\STS\\SpringBootTest\\src\\main\\resources\\static\\assets\\images\\logo1.jpg");
			 Image img = Image.getInstance("/opt/logo.jpg");
			im.scaleToFit(100, 100);
			img.scaleAbsolute(80f, 80f);
			im.scaleAbsolute(80f, 80f);
		    im.setAbsolutePosition(430f, 730f);
			document.add(img);
		    document.add(im);
			Font font = FontFactory.getFont(FontFactory.HELVETICA_BOLD);
			Font font1 = FontFactory.getFont(FontFactory.HELVETICA_BOLD);
